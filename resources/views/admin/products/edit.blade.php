<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-slate-800 leading-tight uppercase tracking-tight">
            {{ __('Edit Product: ') }} <span class="text-red-600">{{ $product->name }}</span>
        </h2>
    </x-slot>
                ...
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-bold p-4" required>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea name="description" rows="4" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-medium p-4">{{ old('description', $product->description) }}</textarea>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            @if($product->image)
                                <div class="mb-4">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="Current Image" class="w-24 h-24 object-cover rounded-xl border border-slate-200 shadow-sm">
                                </div>
                            @endif
                            ...
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Uploading a new image will replace the current one.</p>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price', $product->price) }}" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-bold p-4" required>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price', $product->sale_price) }}" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-bold p-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" value="{{ old('quantity', $product->quantity) }}" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-bold p-4" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="w-full rounded-xl bg-slate-50 border-slate-200 text-slate-800 shadow-inner focus:border-red-500 focus:ring-red-500 transition-all font-bold p-4">
                        ...
                        <div class="col-span-1 md:col-span-2 flex space-x-6 items-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
                            <label class="flex items-center space-x-3 cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-6 h-6 rounded-md border-slate-300 text-red-600 focus:ring-red-500">
                                <span class="text-sm font-black uppercase tracking-widest text-slate-700">Active Listing</span>
                            </label>
                            
                            <label class="flex items-center space-x-3 cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }} class="w-6 h-6 rounded-md border-slate-300 text-orange-500 focus:ring-orange-500">
                    ...
                    <div class="pt-6 border-t border-slate-100 flex justify-end space-x-4">
                        ...
                        <button type="submit" class="px-8 py-4 bg-gradient-to-r from-red-600 to-orange-500 text-white rounded-xl font-black uppercase tracking-widest text-sm hover:from-red-500 hover:to-orange-400 shadow-xl shadow-red-500/30 transform hover:-translate-y-1 transition">Update Product</button>
                    </div>
                </form>
            </div>
        </div>
